...
## Testing notes
- Run the suite with `.venv/bin/python -m pytest -q` (currently **142 passing, 0 failing**), then `./scripts/lint_pep8.sh`. The pre-commit hook runs the PEP 8 check and blocks the commit on failure.
...
## Permanent deletion and schema cascades
- Soft delete only flips `decks.is_deleted` / `deleted_at` (`pages.delete_deck`, `deck_accesses`). Nothing hard-deleted a record until `app/services/purge.py` (`purge_deck`, `purge_bulk_upload`).
- Purge issues dependency-ordered deletes (children before parents, parent row last), guards on state (a deck must already be soft-deleted; refuse while a job is running or a file is processing) and cleans up stored objects (deck media prefix, revision-note keys, `bulk-ai-upload/<id>/`, `bulk_uploads/<id>/`). Decks a purged upload generated are deliberately kept — they are real study content.
- Admin UI: `GET /settings/deleted-decks` (system admin only) lists soft-deleted decks with a "Delete permanently" button behind a reusable `<dialog>` confirmation; `POST /settings/decks/{id}/purge`. The Settings index card shows the count. Finished uploads get a Delete button in `settings/jobs.html` that reuses the retry-modal JS and posts to `POST /api/v1/bulk-ai-upload/{id}/purge`.
- **Cascade rule** (migration `0031_mcq_fk_cascade`): every table owned by a deck must reference its parent with `ondelete="CASCADE"` in the model **and** be listed in that migration's `CASCADE_FKS`. Twelve FKs were `NO ACTION` before, which is why the purge had to delete children by hand. When adding a deck-owned table: declare the cascade, add it to `purge.py`, and add it to `CASCADE_FKS`.
- `mcq_generations` had a model and live route code (`content.py`) but no migration ever created it, so MCQ generation raised `UndefinedTable` on any database built from migrations — `0031` creates it. The purge still tolerates an absent table (`_table_present`) so a missing table can never abort the delete transaction.
- Migration mechanics: constraint names are read from `pg_constraint` (earlier migrations created them unnamed); `downgrade()` restores `NO ACTION` and drops the table. Code — including `alembic/versions/` — is **baked into the image**: `docker compose create` reuses the old image, so a rebuild is mandatory before a new migration runs (the first QA restart silently ran nothing because of this).
- Known gap (pre-existing, not fixed): `process_concept_map_revision` imports `generate_concept_map_revision_pdf` from `app.services.concept_map`, but that function is defined nowhere, so every concept-map revision PDF job fails with an ImportError captured into `job.error_message`.
